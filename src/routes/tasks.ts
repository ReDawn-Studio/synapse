...
export default async function tasksRoutes(fastify: FastifyInstance) {
  ...
  // 创建任务
  fastify.post('/', async (request: FastifyRequest<{ Body: { channel_id: string; title: string; description?: string; priority?: 'low' | 'medium' | 'high' | 'urgent'; assigned_to?: string[]; due_at?: string } }>, reply: FastifyReply) => {
    const { channel_id, title, description, priority = 'medium', assigned_to = [], due_at } = request.body;
    ...
    const result = await db
      ...
      .returning(['id', 'channel_id', 'title', 'description', 'status', 'priority', 'assigned_to', 'due_at', 'created_by', 'created_at', 'updated_at'])
      .executeTakeFirst();
  ...
  // 更新任务状态
  fastify.patch('/:id', async (request: FastifyRequest<{ Params: { id: string }; Body: { status?: 'pending' | 'in_progress' | 'done' | 'failed'; description?: string } }>, reply: FastifyReply) => {
    const { id } = request.params;
    const { status, description } = request.body;
    const user = request.user as { bot_id: string };

    // 获取当前任务状态
    const currentTask = await db.selectFrom('tasks').where('id', '=', id).select(['status', 'description']).executeTakeFirst();
    ...
    if (status && status !== currentTask.status) {
      await db.insertInto('task_updates').values({
        task_id: id,
  ...
  fastify.delete('/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    ...
    // 检查是否是创建者
    const task = await db.selectFrom('tasks').where('id', '=', id).select('created_by').executeTakeFirst();
    ...
    if (task.created_by !== user.bot_id) {
      return reply.code(403).send({ error: 'FORBIDDEN', message: 'Only task creator can delete' });
    }
