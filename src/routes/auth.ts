...
export default async function authRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (
      request: FastifyRequest<{
        Body: { name: string; description?: string; public_key?: string };
      }>,
      reply: FastifyReply
    ) => {
      ...
      if (!name || name.length < 3 || name.length > 32) {
        return reply
          .code(400)
          .send({ error: 'BAD_REQUEST', message: 'Bot name must be 3-32 characters' });
      }
  ...
  fastify.post(
    '/login',
    async (
      request: FastifyRequest<{ Body: { name: string; bot_secret: string } }>,
      reply: FastifyReply
    ) => {
      ...
      if (!bot) {
        return reply
          .code(401)
          .send({ error: 'UNAUTHORIZED', message: 'Invalid bot name or secret' });
      }
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Body: { refresh_token: string } }>, reply: FastifyReply) => {
      ...
      if (!refresh_token.startsWith('synapse_rt_')) {
        return reply
          .code(401)
          .send({ error: 'UNAUTHORIZED', message: 'Invalid refresh token format' });
      }

      // 查找 token 记录
      const tokens = await db.selectFrom('bot_refresh_tokens').selectAll().execute();
      ...
      for (const tokenRecord of tokens) {
        ...
        if (isValid) {
          ...
          if (new Date(tokenRecord.expires_at) < new Date()) {
            return reply
              .code(401)
              .send({ error: 'UNAUTHORIZED', message: 'Refresh token expired' });
          }
