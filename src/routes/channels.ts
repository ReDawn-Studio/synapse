...
export default async function channelsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (
      ...
      if (!result) {
        return reply
          .code(500)
          .send({ error: 'INTERNAL_ERROR', message: 'Failed to create channel' });
      }
  ...
  fastify.delete(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      if (channel.created_by !== user.bot_id) {
        return reply
          .code(403)
          .send({ error: 'FORBIDDEN', message: 'Only channel creator can delete' });
      }
  ...
  fastify.get(
    ...
    async (
      request: FastifyRequest<{
        Params: { id: string };
        Querystring: { limit?: number; offset?: number; since?: number };
      }>,
      reply: FastifyReply
    ) => {
      const { id: channel_id } = request.params;
  ...
  fastify.delete(
    ...
    async (
      ...
      if (message.bot_id !== user.bot_id) {
        return reply
          .code(403)
          .send({ error: 'FORBIDDEN', message: 'Only message sender can delete' });
      }
